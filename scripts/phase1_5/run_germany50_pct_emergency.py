...
OUT = ROOT/"results/official_germany50_pct_emergency"; OUT.mkdir(parents=True, exist_ok=True)
TOPO="germany50"; LO,HI=0,288; FP,FD=0.9677,0.0064; GNN_MS=27; SW=60
...
def run(pct,cap,pu,d,ds,pl_full,caps,cycles=SW):
    ...
    for t in range(LO,min(LO+cycles,HI)):
...
def main():
    ...
    print("Germany50 % EMERGENCY (zero-shot, no retraining). bar=old report PR0.9677/DB0.0064",flush=True)
    rows=[]
    for pct in [50,60,70,75,80]:
        for cap in [2000]:
            for pu in [3,4]:
