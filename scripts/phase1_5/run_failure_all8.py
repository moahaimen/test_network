...
SUB = OUT / "FAILURE_VALIDATION_ITER2_ALL8"; SUB.mkdir(parents=True, exist_ok=True)
...
TOPOS = {"abilene": (2016, 2036), "geant": (672, 692), "cernet": (200, 220), "sprintlink": (200, 220),
         "tiscali": (200, 220), "ebone": (200, 220), "germany50": (0, 20), "vtlwavenet2011": (0, 20)}
GNN_MS = {"abilene": 3, "geant": 7, "cernet": 22, "sprintlink": 27, "tiscali": 33, "ebone": 12, "germany50": 26, "vtlwavenet2011": 140}; CYC = 20
...
pc = pd.DataFrame(allrows); pc.to_csv(SUB / "failure_all8_per_cycle.csv", index=False)
...
summ = pd.DataFrame(srow); summ.to_csv(SUB / "failure_all8_summary.csv", index=False)
...
pd.DataFrame(drow).to_csv(SUB / "failure_all8_disconnect_detail.csv", index=False)
...
ax.set_xlabel("MLU under failure"); ax.set_ylabel("CDF"); ax.set_title("Failure MLU CDF (Iter2 current method)"); ax.legend(fontsize=6,ncol=2); ax.grid(alpha=.3); f.tight_layout(); f.savefig(SUB/"failure_all8_mlu_cdf.png",dpi=130); plt.close(f)
f,ax=plt.subplots(figsize=(6,3.3)); dbm=[pc[pc.scenario==sc].DB.mean() for sc in SCENARIOS]
ax.bar(range(len(SCENARIOS)), dbm, color="#e08a3b"); ax.set_xticks(range(len(SCENARIOS))); ax.set_xticklabels([s.replace('_','\n') for s in SCENARIOS], fontsize=6); ax.set_ylabel("Mean DB"); ax.set_title("Failure DB by scenario (Iter2)"); ax.grid(axis='y',alpha=.3); f.tight_layout(); f.savefig(SUB/"failure_all8_db_by_scenario.png",dpi=130); plt.close(f)
...
json.dump({"method":"Topology-Agnostic Bottleneck-Ranking DDQN (Iter2)","controller":"Double-DQN argmax-Q",
    ...
    "note":"Real failure rerun on the frozen Iter2 controller."}, open(SUB/"failure_all8_audit.json","w"), indent=2)
